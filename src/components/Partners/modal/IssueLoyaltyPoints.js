...
function IssueLoyaltyPoints({submit, id, setPoints, loading, show, onClose}) {
  if(!show) {return null}

  return (
    <div onClick={onClose} className=' fixed top-0 left-0 w-full h-screen z-10 bg-[rgba(0,0,0,0.4)] text-xs md:text-base '>
        <div onClick={e => e.stopPropagation()} className='bg-[#fefefe] mx-auto rounded-lg my-[5%] m max-w-xs md:max-w-[300px] p-5 border-none outline-none'>
           <div className='relatives mt-4 text-xs '>
                <h2 className='font-bold'>Issue Loyalty Points - fill details below</h2>
...
export default IssueLoyaltyPoints